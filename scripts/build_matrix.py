...
# по цене принимается именно по ним.
HEADER_NOTES = {
    "Предметов в SKU":
        "Сколько изделий покупатель получает в одной нашей позиции.\n"
        "Нужно, чтобы не сравнивать цену набора из 6 салфеток с ценой одной.",
    "Рынок: сопоставимых карточек":
        "На скольких карточках выдачи посчитан бенчмарк.\n"
        "Берутся только карточки того же типа, размера и комплектности.\n"
        "Меньше 15 — цифры рядом хрупкие, решение на них опирать рано.",
    "Рынок: медиана":
        "Середина цен сопоставимых карточек: половина дешевле, половина дороже.\n"
        "Не среднее — одна карточка за 15 000 ₽ её не сдвинет.\n"
        "Это и есть «цена, которую покупатель считает нормальной».",
    "Рынок: 25%":
        "Нижний квартиль: четверть карточек стоит дешевле.\n"
        "Граница, ниже которой начинается эконом-сегмент.",
    "Рынок: 75%":
        "Верхний квартиль: четверть карточек стоит дороже.\n"
        "Вместе с колонкой 25% задаёт коридор, где живёт половина рынка.",
    "Рынок: медиана за 1 шт":
        "Медианная цена, приведённая к одному предмету.\n"
        "Главная колонка для наборов: только она сравнивает честно.",
    "Рынок: ср. отзывов":
        "Средние отзывы на сопоставимой карточке — косвенная оценка спроса.\n"
        "Много отзывов = в сегменте покупают, а не просто висят карточки.",
    "Таргет, ₽":
        "Наша цена. Правится здесь или в data/sku_matrix.csv.",
    "Таргет за 1 шт":
        "Наша цена, делённая на количество предметов в SKU.\n"
        "Сравнивать нужно с колонкой «Рынок: медиана за 1 шт».",
    "К медиане":
        "Насколько наша цена отличается от медианы рынка.\n"
        "0% — ровно по рынку, +20% — дороже на пятую часть.\n"
...
def write_header(ws, headers: list[tuple[str, int]], row: int = 1) -> None:
    for i, (name, width) in enumerate(headers, start=1):
        ...
        cell.border = BORDER
        # примечание к заголовку: наводишь мышь — видишь, что это за цифра
        if name in HEADER_NOTES:
            comment = Comment(HEADER_NOTES[name], "Анализ рынка")
            comment.width, comment.height = 320, 130
